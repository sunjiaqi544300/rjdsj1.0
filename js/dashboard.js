...
$(function() {
  ...
  var subjectUsageAnimation = function subjectUsageAnimation(
    chart,
    seriesIndex
  ) {
    ...
    tooltip: {
      ...
      formatter: function formatter(obj) {
        return (
          obj[0].name +
          '<br>教材用户：' +
          ((obj[0].value / textbookTop5Count) * 100).toFixed(2) +
          '%'
        );
      }
    },
          ...
          normal: {
            color: function color() {
              var randIndex = Math.round(
                (hotAppColor.length - 1) * Math.random()
              );
  ...
  window.onload = function() {
    ...
    subjectUsageCharts = echarts.init(
    ...
    $('.map-charts')
      .height($('.box-center-a').height())
      .width($('.box-center-a').width());
    mapCharts = echarts.init(document.querySelector('.map-charts'));
    mapCharts.setOption(mapOpts); // hotSubjectTop10Charts

    hotSubjectTop10Charts = echarts.init(
      document.querySelector('.hot-subject-top10-charts')
    );
    ...
    textbookTop5Charts = echarts.init(
      document.querySelector('.textbook-top5-charts')
    );
    textbookTop5Charts.setOption(textbookTop5ChartsOpts); // realtimeUser

    realtimeUserCharts = echarts.init(
      document.querySelector('.realtime-user-charts')
    );
    realtimeUserCharts.setOption(realtimeUserChartsOpts); // userTrendCharts

    userTrendCharts = echarts.init(
      document.querySelector('.user-trend-charts')
    );
    ...
    function bindEvents() {
      function reqFullScreenCb () {
        console.log('dashboard reqFullScreenCb');
        // TODO: resize
      ...
      // resize
      function resize() {
        $('.charts-box').height($('.box-aside').height() - 28)
          .width($('.box-aside').width());
        $('.map-charts')
        .height($('.box-center-a').height())
        .width($('.box-center-a').width());
        $('.realtime-user-charts')
        .height($('.box-center-b').height() - 28)
        .width($('.box-center-b').width());

(function () {
  ...
  // 用户购买转化
  var ymgmzhCharts = echarts.init(
    document.getElementById('ymgmzh')
  );
  var ymgmzhOptions = {
  ...
  // 日均活跃用户占历史累计比
  var rjhyhCharts = echarts.init(
    document.getElementById('rjhy')
  );
  var value = 0.22;
  var data = [];
  data.push(value);
  ...
  //日均新增用户占日均活跃比
  var rjxzCharts = echarts.init(
    document.getElementById('rjxz')
  );
  ...
  rjxzCharts.setOption(rjxzOptions);

  // 用户渠道分布
  var yhqdfbCharts = echarts.init(
    document.getElementById('yhqdfb')
  );
  var ydata = [
    {
      name: '渠道一',
  ...
  // 区域地图
  var yhdqfbCharts = echarts.init(
    document.getElementById('yhdqfb')
  );
  ...
  //区域地图 -地图数据
  var mapdata = [
    { name: '北京', miaoshu: 'nihao', value: randomData() },
    { name: '天津', miaoshu: 'nihao', value: randomData() }
  ]
  var yhdqfbOptions = {
    backgroundColor: '#fff',
    grid: {
  ...
  yhdqfbCharts.setOption(yhdqfbOptions);

  // 今日实时动态数据
  var jrssdtsjCharts = echarts.init(
    document.getElementById('jrssdtsj')
  );
  //今日实时数据动态数据
  ...
  jrssdtsjCharts.setOption(jrssdtsjOptions);

  // 指标变化趋势 
  var zbbhqsCharts = echarts.init(
    document.getElementById('zbbhqs')
  );
  ...
  var zbbhqsOptions = {
    ...
    dataZoom: [{
      show: xdata.length>30?true:false, //控制下面滚动条
      height: 10,
      xAxisIndex: [
        0
      ],
